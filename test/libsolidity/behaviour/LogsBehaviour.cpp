...
bool LogsBehaviour::after(TestFunctionCall const& _call, bool _artificial)
{
	(void) _artificial;
	std::cout << "[logs::after]  " << &_call << ": " << _call.call().signature << std::endl;

	m_producedLogs[&_call] = m_executionFramework->recordedLogs();
	...
	if (producer)
	{
		...
		return m_touchedLogs[&producer->call()].size() >= m_producedLogs[producer].size();
	}
	return true;
}

void LogsBehaviour::generateExpectation(TestFunctionCall const&, std::ostream&) {}

bool LogsBehaviour::isIncomplete(const TestFunctionCall& call)
{
	(void) call;
	return false;
}
